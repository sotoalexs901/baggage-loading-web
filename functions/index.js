...
function assertManager(data) {
  const role = normalizeRole(data && data.userRole);
  const allowed = [
    'station_manager',
    'duty_manager',
    'duty_managers',
...
function buildActor(data) {
  return {
    ...
    role: data && data.userRole ? data.userRole : null,
    fullName: data && data.employeeFullName ? data.employeeFullName : null,
    operationalPosition:
      data && data.operationalPosition ? data.operationalPosition : null,
    operationalPositionLabel:
      data && data.operationalPositionLabel
...
async function deleteBagTagDocumentAndEvents(tagDocRef) {
  ...
  try {
    deletedEvents = await deleteCollectionDocs(tagDocRef.collection('events'));
  } catch (error) {
    console.warn('Unable to delete bag tag events:', tagDocRef.id, error);
  }

  try {
    await tagDocRef.delete();
  } catch (error) {
    console.warn('Unable to delete bag tag document:', tagDocRef.id, error);
  }
...
async function deleteGlobalBagTagsForFlight(flightId) {
  ...
  const candidateFields = ['flightId', 'currentFlightId', 'lastFlightId'];
  const refs = new Map();

  for (const field of candidateFields) {
    try {
      const snap = await bagTagsRef.where(field, '==', flightId).get();
      snap.docs.forEach((docSnap) => refs.set(docSnap.ref.path, docSnap.ref));
    } catch (error) {
      console.warn(`Bag tag lookup failed for ${field}:`, error);
    }
  }
  ...
  for (const tagRef of refs.values()) {
    deletedBagTagEvents += await deleteBagTagDocumentAndEvents(tagRef);
    deletedBagTags += 1;
  }

  return { deletedBagTags, deletedBagTagEvents };
}
      ...
      try {
        ...
        for (const file of files) {
          try {
            ...
            console.warn('Storage delete failed:', file.name, error);
          }
        }
      } catch (error) {
        console.warn('Storage prefix lookup failed:', prefix, error);
      }
    }
  } catch (error) {
...
async function deleteFlightData(flightId) {
  ...
  for (const name of subcollections) {
    deletedSubcollections[name] = await deleteCollectionDocs(
      flightRef.collection(name)
    );
  }

  const bagTagCleanup = await deleteGlobalBagTagsForFlight(flightId);
  const deletedStorageFiles = await deleteStorageForFlight(flightId);
...
exports.reopenFlight = functions
  ...
  .https.onCall(async (data) => {
    ...
    const flightId = String((data && data.flightId) || '').trim();
    if (!flightId) {
      throw new functions.https.HttpsError(
        'invalid-argument',
    ...
    batch.set(reportRef, {
      type: 'REOPEN_FLIGHT',
      reason:
        (data && data.reason) || 'Flight reopened from Flights page.',
...
exports.deleteFlightCascade = functions
  ...
  .https.onCall(async (data) => {
    ...
    try {
      ...
      throw new functions.https.HttpsError(
        'internal',
        (error && error.message) || 'Unable to delete flight.'
      );
    }
  });
...
exports.deleteLoadedFlightsByMonth = functions
  ...
  .https.onCall(async (data) => {
    ...
    try {
      ...
      const loadedFlights = snap.docs.filter((docSnap) => {
      ...
      const deleted = [];
      let totalBagTags = 0;
      let totalBagTagEvents = 0;
      let totalStorageFiles = 0;

      for (const flightDoc of loadedFlights) {
        const result = await deleteFlightData(flightDoc.id);
        deleted.push(result);
        totalBagTags += result.deletedBagTags || 0;
        totalBagTagEvents += result.deletedBagTagEvents || 0;
        totalStorageFiles += result.deletedStorageFiles || 0;
      }
      ...
      return {
        ...
        deletedBagTags: totalBagTags,
        deletedBagTagEvents: totalBagTagEvents,
        deletedStorageFiles: totalStorageFiles,
