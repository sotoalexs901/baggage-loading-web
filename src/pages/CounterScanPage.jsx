...
function getErrorMessage(error, fallback) {
  return error?.message || fallback || "Unknown error";
}
...
export default function CounterScanPage({
  ...
  const logCounterIncident = async ({
    action,
    status = "ERROR",
    severity = "MEDIUM",
    errorType = null,
  ...
  const validateOtherFlight = async (tag) => {
    ...
    const existingFlightId =
      existing?.currentFlightId ||
      existing?.flightId ||
      null;

    if (!existingFlightId || existingFlightId === flightId) {
      return { ok: true };
    }

    const registeredFlightRef = doc(
      db,
      "flights",
      existingFlightId
    );

    const registeredFlightSnap = await getDoc(
      registeredFlightRef
    );

    if (!registeredFlightSnap.exists()) {
      console.warn(
        "Stale bag tag flight reference detected:",
        {
          tag,
          oldFlightId: existingFlightId,
          oldFlightNumber:
            existing?.flightNumber ||
            existing?.currentFlightNumber ||
            null,
          oldFlightDate: existing?.flightDate || null,
          newFlightId: flightId,
          newFlightNumber: flight?.flightNumber || null,
          newFlightDate: flight?.flightDate || null,
        }
      );
    ...
    const registeredFlight = {
      id: registeredFlightSnap.id,
      ...registeredFlightSnap.data(),
    };
    ...
    return {
      ...
      message:
        ...
        `(${flight?.flightDate || "-"})\n` +
        `Registered flight: ${
          registeredFlight?.flightNumber ||
          existing?.flightNumber ||
          existing?.currentFlightNumber ||
          existingFlightId
        } (${
          registeredFlight?.flightDate ||
          existing?.flightDate ||
          "-"
        })\n\n` +
  ...
  const saveCounterScan = async (tag) => {
    ...
    await setDoc(
      doc(db, "bagTags", tag),
      {
        ...
        flightNumber: flight?.flightNumber || null,
        currentFlightNumber: flight?.flightNumber || null,
        flightDate: flight?.flightDate || null,
        gate: flight?.gate || null,
        bagType: selectedBagType,
        bagTypeLabel: getBagTypeLabel(selectedBagType),
  ...
  const handleScanSubmit = async (forcedValue) => {
    ...
    const timer = startSystemTimer();

    if (!flight) {
      const message = "Flight not loaded.";
      setErr(message);
      ...
      await logCounterIncident({
        ...
        status: "ERROR",
        severity: "HIGH",
        errorType: "FLIGHT_NOT_AVAILABLE",
        message,
    ...
    if (tag.length !== MIN_TAG) {
      ...
      await logCounterIncident({
        ...
        durationMs: timer.elapsed(),
        metadata: {
          receivedLength: tag.length,
          requiredLength: MIN_TAG,
        },
      });
    ...
    try {
      ...
      if (!check.ok) {
        ...
        await logCounterIncident({
          ...
          status: "WARNING",
          severity: "HIGH",
          errorType: "WRONG_FLIGHT",
          message: check.message,
          tag,
          durationMs: timer.elapsed(),
          metadata: {
            registeredFlightId:
              check?.existing?.flightId ||
              check?.existing?.currentFlightId ||
              null,
            registeredFlightNumber:
              check?.existing?.flightNumber || null,
            registeredFlightDate:
              check?.existing?.flightDate || null,
          },
        });
      ...
      console.error("Counter scan error:", error);
